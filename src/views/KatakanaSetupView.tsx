...
import { KATAKANA_ROWS, KATAKANA_DAKUTEN_ROWS, KATAKANA_COMPOUND_ROWS, KATAKANA_ALL_ROW_GROUPS, KATAKANA_ALL_CHARS } from "../dataKatakana";
...
// ── Design tokens (mismo sistema que HiraganaSetupView, acento azul) ────────

const BLUE        = "#2F6FE4";
const BLUE_DARK    = "#1D4FB0";
const BLUE_LIGHT   = "#E7EFFD";
const CORAL        = "#E85D3A";
...
const TAB_ROWS: Record<TabId, typeof KATAKANA_ROWS> = { basic: KATAKANA_ROWS, dakuten: KATAKANA_DAKUTEN_ROWS, compound: KATAKANA_COMPOUND_ROWS };
...
const MODE_SUBTITLE: Record<SessionMode, string> = {
  recognition: "Ves el katakana, escribes el romaji.",
  production: "Ves el romaji, produces el katakana.",
  both: "Mezcla de ambos modos.",
};

const LAST_SESSION_KEY = "katakana_last_session";
...
export default function KatakanaSetupView({
  progress,
  ...
  const allPairsSelected = selectedPairs.size === KATAKANA_CONFUSED_PAIRS.length;
  ...
  function toggleAllPairs() {
    KATAKANA_CONFUSED_PAIRS.forEach((_, idx) => {
  ...
  function handleStartSession() {
    ...
    launchSession(poolForSelected, () =>
      startSession(poolForSelected, sessionLength === "all" ? availableItems.length : Math.min(sessionLength, availableItems.length), sessionMode)
    );
  }
  ...
  function handleContinue() {
    ...
    const pool  = KATAKANA_ALL_CHARS.filter((c) => lastSession.rowIds.includes(c.row));
    ...
    <div>
      ...
      {lastSession && (
        <div
          ...
          style={{ background: `linear-gradient(135deg, ${BLUE}, ${BLUE_DARK})`, overflow: "visible" }}
        >
          ...
          <div className="flex flex-wrap gap-1.5 mt-2 pr-14">
            {lastSession.rowIds.map((id) => {
              const row = KATAKANA_ALL_ROW_GROUPS.find((r) => r.id === id);
              if (!row) return null;
              return (
      ...
      <div className="mt-8">
        ...
        <div className="flex gap-1 rounded-xl p-1 mt-3" style={{ backgroundColor: "#EEF3FD" }}>
          {(Object.keys(TAB_LABEL) as TabId[]).map((tab) => (
            <button
              ...
              style={{ color: activeTab === tab ? BLUE_DARK : TEXT_SECOND }}
            >
              {TAB_LABEL[tab]}
            </button>
          ))}
        ...
        <div className="flex items-center justify-between mt-4 mb-2">
          ...
          <button onClick={toggleSelectAllActive} className="text-xs font-semibold" style={{ color: BLUE }}>
        ...
        <div className="grid grid-cols-2 gap-2">
          {activeRows.map((row) => {
            const stats      = rowStats(progress, row.id, KATAKANA_ALL_ROW_GROUPS);
            ...
            return (
              <button
                ...
                className={`relative text-left rounded-2xl border-2 p-3 transition-colors ${!isSelected ? "hover:border-[#B9CDF7] hover:bg-[#F7FAFF]" : ""}`}
                style={isSelected
                  ? { backgroundColor: BLUE_LIGHT, borderColor: BLUE, boxShadow: "0 4px 14px rgba(47,111,228,0.18)" }
                  : { backgroundColor: "#FFFFFF", borderColor: BORDER }
                }
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="text-2xl" style={{ fontFamily: "'Noto Sans JP', sans-serif", color: isSelected ? BLUE_DARK : TEXT_MAIN }}>
                    {row.chars[0].kana}
                  </span>
                  {isSelected ? (
                    <span className="w-[18px] h-[18px] rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: BLUE }}>
                ...
                <div className="text-xs mt-1" style={{ color: isSelected ? BLUE_DARK : TEXT_SECOND }}>
                  {shortRowLabel(row.title)}
                </div>
              </button>
            );
        ...
        <div className="flex gap-2 mt-5">
          {(["recognition", "production", "both"] as SessionMode[]).map((m) => (
            <button
              ...
              style={sessionMode === m
                ? { borderColor: BLUE, backgroundColor: BLUE_LIGHT, color: BLUE_DARK }
      ...
      <div className="h-px my-8" style={{ backgroundColor: "#EEF3FD" }} />
      ...
      <div>
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: TEXT_MUTED }}>Los katakana que más se confunden</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-3">
          {KATAKANA_CONFUSED_PAIRS.map((group, idx) => {
      ...
      {/* ── Sesión de préstamos (loanwords) ── */}
      <div className="mt-8 pt-6" style={{ borderTop: "1px solid #EEF3FD" }}>
        <span className="text-sm font-medium" style={{ color: TEXT_MAIN }}>Préstamos (loanwords)</span>
        ...
        <button
          ...
          style={{ borderColor: BLUE, color: BLUE, backgroundColor: "#FFFFFF" }}
        >
          <Play size={16} /> Sesión de préstamos
